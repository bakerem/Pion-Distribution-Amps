...
fits_dict = {"Z5": Z5_fits, "T5": T5_fits}
...
for i in ["Z5", "T5"]:
    for Pz in range(4,10):
        ...
        real_fit = real_state2ratio(100,i,fits_dict[i][f"Pz = {Pz}"][1,0], fits_dict[i][f"Pz = {Pz}"][3,0])
        imag_fit = imag_state2ratio(100,i,fits_dict[i][f"Pz = {Pz}"][6,0], fits_dict[i][f"Pz = {Pz}"][8,0])

        real_fit_err1 =np.abs(real_state2ratio(100,i,
                            fits_dict[i][f"Pz = {Pz}"][1,0], 
                            fits_dict[i][f"Pz = {Pz}"][3,0]+np.sqrt(fits_dict[i][f"Pz = {Pz}"][4,0])) 
                                - real_fit)
        real_fit_err2 =np.abs(real_state2ratio(100,i,
                            fits_dict[i][f"Pz = {Pz}"][1,0]+np.sqrt(fits_dict[i][f"Pz = {Pz}"][2,0]), 
                            fits_dict[i][f"Pz = {Pz}"][3,0]) 
                                - real_fit)
        real_fit_err = np.sqrt(real_fit_err1**2 + real_fit_err2**2)

        imag_fit_err1 =np.abs(imag_state2ratio(100,i,
                            fits_dict[i][f"Pz = {Pz}"][6,0], 
                            fits_dict[i][f"Pz = {Pz}"][8,0]+np.sqrt(fits_dict[i][f"Pz = {Pz}"][9,0])) 
                                - imag_fit)
        imag_fit_err2 =np.abs(imag_state2ratio(100,i,
                            fits_dict[i][f"Pz = {Pz}"][6,0]+np.sqrt(fits_dict[i][f"Pz = {Pz}"][7,0]), 
                            fits_dict[i][f"Pz = {Pz}"][8,0]) 
                                - imag_fit)
