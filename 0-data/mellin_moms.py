...
for init_char in ["Z5","T5"]:
    ...
    for bz_min in range(bz_min_low, bz_min_up):
        for bz_max in range(bz_max_low, bz_max_up):
            ...
            # the matrix elements
            def ratio(bzPz, mm2, mm4,mm6,mm8,h0,l):
                bz, Pz = bzPz_range
                num   = m_ope(bz/a, mm2, mm4, mm6, mm8, l, h0, 0, 2*N_max, Nh, phys_p(a,Pz), init_char)
                denom = m_ope(bz/a, mm2, mm4, mm6, mm8, l, h0, 0, 2*N_max, Nh, phys_p(a,P0), init_char)
            ...
            if save:
                save_path = f"final_results/2_state_matrix_results_jack/{init_char}/mellin_moms"
                os.makedirs(save_path, exist_ok=True)
