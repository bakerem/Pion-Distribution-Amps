...
from functions import x_dep1_m_ope, phys_p, alpha_func
                                ...
                                for Pz_min in range(Pz_min_low, Pz_min_up):
                                    for Pz_max in range(Pz_max_low, Pz_max_up):
                                        ...
                                        if Nh == 0 and Nl == 0:
                                            ...
                                            def ratio(bzPz, alpha,l ):
                                                bz, Pz = bzPz_range
                                                num   = x_dep1_m_ope(bz/a, alpha, l, 0, 0, 2*N_max, Nh, phys_p(a,Pz), alpha_s, mu, init_char)
                                                denom = x_dep1_m_ope(bz/a, alpha, l, 0, 0, 2*N_max, Nh, phys_p(a,P0), alpha_s, mu, init_char)
                                                ratio_result = num/denom
                                                return np.real(ratio_result)
                                            ...
                                            def ratio(bzPz, alpha, h0, l ):
                                                bz, Pz = bzPz_range
                                                num   = x_dep1_m_ope(bz/a, alpha, l, h0, 0, 2*N_max, Nh, phys_p(a,Pz), alpha_s, init_char)
                                                denom = x_dep1_m_ope(bz/a, alpha, l, h0, 0, 2*N_max, Nh, phys_p(a,P0), alpha_s, init_char)
                                                ratio_result = num/denom
                                                return np.real(ratio_result)
                                            ...
                                            popt, pcov = curve_fit(ratio,
                                                            bzPz_range,
                                                            matrix_els,
                                                            bounds = [(0, -0.01, -0.1),(np.inf, 0.01, 0.1)],
                                                            sigma=cov)
                                        ...
                                        if save:
                                            ...
                                            os.makedirs(save_path, exist_ok=True)
                                            np.save(f"{save_path}/mellin_alpha_chi2_Nmax{N_max}_h{Nh}_l{Nl}_P0{P0}_resum_k"+"%.2f"%kappa+".npy", chi2s)
                                            np.save(f"{save_path}/mellin_alpha_Nmax{N_max}_h{Nh}_l{Nl}_P0{P0}_resum_k"+"%.2f"%kappa+".npy",alpha)
                                            np.save(f"{save_path}/mellin_alpha_err_Nmax{N_max}_h{Nh}_l{Nl}_P0{P0}_resum_k"+"%.2f"%kappa+".npy", alpha_err)
