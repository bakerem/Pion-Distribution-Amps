    ...
    for init_char in ["Z5", "T5"]:
        ...
        for bz in range(0,33):
            (real_ratio_means,
                ...
                real_sample_errs,
                imag_samples,
                imag_sample_errs) = read_data(Ns, Nt, init_char, P0, bz, smear)
        ...
        if save:
            np.save(f"{save_path}/real_matrix_elj.npy", real_matrix_el_js)
            np.save(f"{save_path}/real_matrix_el.npy", real_matrix_el)
            np.save(f"{save_path}/real_matrix_err.npy", real_matrix_err)
            np.save(f"{save_path}/imag_matrix_elj.npy", imag_matrix_el_js)
        ...
        if real:
            ...
            plt.text(4, 1, r"$P_0$ " + "= %.2f GeV" %phys_p(a,P0))
            if save:
            ...
            plt.text(0.25, 0.06, r"$P_0$ " + "= %.2f GeV" %phys_p(a,P0))
            plt.title(f"Renormalized Matrix Elements: {init_char}")
            plt.xlim(0,7)
            plt.ylim(-0.03, 0.08)
            plt.axhline(0,color="black", linestyle="--")
            if save:
                plt.savefig(f"{save_path}/imag_renorm_multi_p_full.pdf")
            plt.show()
