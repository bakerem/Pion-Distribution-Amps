...
df = pd.read_csv(
    f"k0/c2pt_cfgs/64IGSRC_W40_k0.ama.c2pt.PX0PY0PZ{Pz}.real.cfg.csv",
    names=columns,
    dtype=np.float64)
...
def perform_fit(lower_lim, upper_lim, plot=False):
    ...
    if plot == True:
        ...
        plt.title(f"Nz = {Pz}")
        if save == True:
            plt.savefig(f"k0/2state_fit_results/Pz{Pz}_corrfunc.png")
        plt.show()
...
if save == True:
    plt.savefig(f"k0/2state_fit_results/Pz{Pz}window_length{window}.png")
    np.save(f"k0/2state_fit_results/window_arrays/E1_fits_Pz{Pz}.npy", fit_results)
plt.show()
