...
# Create save path
if save == True:
    save_path = "k0/2_state_matrix_results"
    os.makedirs(save_path, exist_ok=True)

# load data into a dictionary with keys given by lattice momentum values
for i in range(0,4):
    Z5_fit = np.load(f"k0/2_state_matrix_results/Z5/Pz{i}/Pz{i}_R.npy")
    T5_fit = np.load(f"k0/2_state_matrix_results/T5/Pz{i}/Pz{i}_R.npy")
...
for i in ["Z5","T5"]:
    for Pz in range(0,4):
        ...
        if i == "Z5":
            real_fit = real_state2ratio(20,i,Z5_fits[f"Pz = {Pz}"][1,0], Z5_fits[f"Pz = {Pz}"][3,0])
            imag_fit = imag_state2ratio(20,i,Z5_fits[f"Pz = {Pz}"][6,0], Z5_fits[f"Pz = {Pz}"][8,0])
            real_fit_err1 =np.abs(real_state2ratio(20,i,
                                Z5_fits[f"Pz = {Pz}"][1,0]+np.sqrt(Z5_fits[f"Pz = {Pz}"][2,0]), 
                                Z5_fits[f"Pz = {Pz}"][3,0]) 
                                 - real_fit)
            real_fit_err2 =np.abs(real_state2ratio(20,i,
                                Z5_fits[f"Pz = {Pz}"][1,0], 
                                Z5_fits[f"Pz = {Pz}"][3,0]+np.sqrt(Z5_fits[f"Pz = {Pz}"][4,0])) 
                                 - real_fit)
            real_fit_err = np.sqrt(real_fit_err1**2 + real_fit_err2**2)

            imag_fit_err1 =np.abs(imag_state2ratio(20,i,
                                Z5_fits[f"Pz = {Pz}"][6,0], 
                                Z5_fits[f"Pz = {Pz}"][8,0]+np.sqrt(Z5_fits[f"Pz = {Pz}"][9,0])) 
                                 - imag_fit)
            imag_fit_err2 =np.abs(imag_state2ratio(20,i,
            ...
            real_fit = real_state2ratio(20,i,T5_fits[f"Pz = {Pz}"][1,0], T5_fits[f"Pz = {Pz}"][3,0])
            imag_fit = imag_state2ratio(20,i,T5_fits[f"Pz = {Pz}"][6,0], T5_fits[f"Pz = {Pz}"][8,0])
...
if save == True:
    plt.savefig(f"{save_path}/0_3_real_f_pi.png")
plt.show()
